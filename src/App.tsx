        ...
        <div className='d-flex flex-column flex-md-row justify-content-center'>
          {dailyWeathers &&
            dailyWeathers?.map(
              (day, index) =>
                index < 7 && (
                  <div onClick={()=>handleSelectDays(index)} className={`day d-flex flex-row flex-md-column m-1 m-lg-2 border rounded align-items-center justify-content-center p-0 p-md-2 ${index === selectedDay && 'active'}`}>
                    <img
                    ...
                    <p>{day.temp.min.toFixed(0)}° / {day.temp.max.toFixed(0)}°</p>
                  </div>
                )
            )}
        </div>
